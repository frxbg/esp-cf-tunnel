/* Small SHA-256 fallback for local HTTP, where Web Crypto is unavailable.
 * Firmware bytes stay in the browser; the ESP receives bounded chunks. */
function sha256Bytes(input) {
  ...
  const view = new DataView(bytes.buffer), words = new Uint32Array(64);
  view.setUint32(bytes.length - 8, Math.floor(input.length / 0x20000000));
  view.setUint32(bytes.length - 4, (input.length * 8) >>> 0);
  const rotate = (n, shift) => (n >>> shift) | (n << (32 - shift));
  for (let pos = 0; pos < bytes.length; pos += 64) {
    for (let i = 0; i < 16; ++i) words[i] = view.getUint32(pos + i * 4);
    for (let i = 16; i < 64; ++i) {
...
if (typeof module !== 'undefined') module.exports = {sha256Bytes};
